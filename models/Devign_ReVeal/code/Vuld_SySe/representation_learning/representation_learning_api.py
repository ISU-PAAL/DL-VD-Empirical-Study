...
class RepresentationLearningModel(BaseEstimator):
    def __init__(self,
        ...
        print("RepresentationLearningModel cuda:", self.cuda)
        self.num_layers = num_layers

    def fit(self, train_x, train_y):
        self.train(train_x, train_y)

    def train(self, train_x, train_y, valid_x, valid_y, model_dir):
        ...
        if self.cuda:
            self.model.cuda(device=torch.device("cuda:0"))
        ...
        train(
            ...
            num_epochs=self.num_epoch, model_dir=model_dir, max_patience=self.max_patience,
            cuda_device=torch.device("cuda:0") if self.cuda else -1,
        )
