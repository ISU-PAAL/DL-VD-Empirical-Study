...
def train(model, dataset, optimizer, num_epochs, model_dir, max_patience=5,
    ...
    try:
        for epoch_count in range(num_epochs):
            ...
            output_batches_generator = tqdm(output_batches_generator, desc=f"epoch {epoch_count}")
            ...
            if epoch_count % valid_every == 0:
                ...
                if vf1 > best_f1:
                    ...
                    torch.save(best_model, os.path.join(model_dir, "RepresentationLearningModel.bin"))
                else:
                    patience_counter += 1
