...
def inputGeneration(nodeCSV, edgeCSV, target, wv, edge_type_map, cfg_only=False):
    ...
    with open(nodeCSV, 'r') as nc:
        ...
        for idx, node in enumerate(nodes):
            ...
            allNodesTokens[nodeKey] = node_split
            nrp = np.zeros(100)
        ...
        for i, node in enumerate(all_nodes_with_edges):
            trueNodeMap[node] = i
            gInput["node_code"].append(allNodesCode[node])
            gInput["node_tokens"].append(allNodesTokens[node])
...
def main():
    ...
    args.src = f'{data_dir}/raw_code/'
    args.wv = f'{data_dir}/wv_models/raw_code.100'
    args.output = f'{data_dir}/full_experiment_real_data/data.jsonlines'
