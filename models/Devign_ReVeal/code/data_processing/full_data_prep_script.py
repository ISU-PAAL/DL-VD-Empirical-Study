...
# base_dir = f'{data_dir}/full_experiment_real_data'
# shards = os.listdir(base_dir)
...
def extract_graph_data(project, portion):
    assert portion in ['full_graph', 'cgraph', 'dgraph', 'cdgraph']

    base_dir='data'
    data_dir = f"{base_dir}/{project}"
    output_dir=f'{data_dir}/full_experiment_real_data_processed/'
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)

    with jsonlines.open(os.path.join(base_dir, project, "full_experiment_real_data/data.jsonlines")) as reader, jsonlines.open(os.path.join(output_dir, project + '-' + portion + '.jsonlines'), "w") as writer:
        for data in tqdm.tqdm(reader):
            ...
            writer.write(code_graph)

# In[8]:


# extract_graph_data('chrome_debian', 'full_graph')
# extract_graph_data('MSR', 'full_graph')
# extract_graph_data('devign', 'full_graph')
import sys
extract_graph_data(sys.argv[1], 'full_graph')


# In[5]:


# extract_token_data('devign', 'vuld')
# extract_token_data('devign', 'syse')
# extract_token_data('devign', 'draper')
...
# extract_token_data('chrome_debian', 'vuld')
# extract_token_data('chrome_debian', 'syse')
# extract_token_data('chrome_debian', 'draper')
...
def reformat_code_line_graph(file_name, code_lines, adjacency_lists, lanel, wv_model_original, wv_model_li, label):
    actual_lines = []
...
def extract_line_graph_data(project):
    base_dir = 'data'
    data_dir = os.path.join(base_dir, project)
    output_dir = os.path.join(data_dir, "full_experiment_real_data_processed")
    
    os.makedirs(output_dir, exist_ok=True)
    code_dir = f'{data_dir}/raw_code/'
    parsed = f'{data_dir}/parsed/tmp/'
    wv_path = f'{data_dir}/wv_models/raw_code.100'
    wv_model_original = Word2Vec.load(wv_path)
    
    data_file = os.path.join(data_dir, "full_experiment_real_data", "data.jsonlines")

    # with jsonlines.open(data_file) as reader, jsonlines.open(os.path.join(output_dir, project + '-line-ggnn.jsonlines'), "w") as writer:
    files = os.listdir(code_dir)
    with jsonlines.open(os.path.join(output_dir, project + '-line-ggnn.jsonlines'), "w") as writer:
        for file_name in tqdm(files):
            label = int(file_name.strip()[:-2].split('_')[-1])
            code_text = read_code_file(code_dir + file_name.strip())
            ...
            data_point = reformat_code_line_graph(file_name, code_text, adjacency_list, label, wv_model_original, wv_model_li, label)
...
# graph_data = extract_line_graph_data("MSR")
# graph_data = extract_line_graph_data("devign_modified")
# graph_data = extract_line_graph_data("combined")
