...
# for val in ${StringArray[@]}; do
    ...
    rm -r "<ANONYMOUS>/<ANONYMOUS>/PLBART/data/codeXglue/code-to-code/defect_prediction/combined/$val2/processed"
    ...
    DATA_DIR_holdout=${HOME_DIR}/data/codeXglue/code-to-code/defect_prediction/combined/$val2
    ...
    function spm_preprocess() {
        ...
        for SPLIT in train valid test; do
            ml-gpu <ANONYMOUS>/<ANONYMOUS>/plbart_venv/bin/python encode.py \
                --model_file ${SPM_DIR}/sentencepiece.bpe.model \
                --src_file $DATA_DIR_holdout/$SPLIT.jsonl \
                --output_dir $DATA_DIR_holdout/processed \
                --split $SPLIT
        done
    ...
    function binarize() {
        ...
        ml-gpu <ANONYMOUS>/<ANONYMOUS>/plbart_venv/bin/python /work/LAS/<ANONYMOUS>-lab/rd<ANONYMOUS>/PLBART/fairseq/fairseq_cli/preprocess.py \
            --only-source \
            --trainpref $DATA_DIR_holdout/processed/train.input0 \
            --validpref $DATA_DIR_holdout/processed/valid.input0 \
            --testpref $DATA_DIR_holdout/processed/test.input0 \
            --destdir $DATA_DIR_holdout/processed/data-bin/input0 \
            --srcdict ${SPM_DIR}/dict.txt \
            --workers 60
        ml-gpu <ANONYMOUS>/<ANONYMOUS>/plbart_venv/bin/python /work/LAS/<ANONYMOUS>-lab/rd<ANONYMOUS>/PLBART/fairseq/fairseq_cli/preprocess.py \
            --only-source \
            --trainpref $DATA_DIR_holdout/processed/train.label \
            --validpref $DATA_DIR_holdout/processed/valid.label \
            --destdir $DATA_DIR_holdout/processed/data-bin/label \
            --workers 60
