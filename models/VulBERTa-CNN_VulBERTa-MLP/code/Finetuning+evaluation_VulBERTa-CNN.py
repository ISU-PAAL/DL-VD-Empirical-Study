...
parser.add_argument("--quit_after_load", action="store_true")
...
pad_id = 1
my_tokenizer.enable_padding(direction='right', pad_id=pad_id, pad_type_id=0, pad_token='<pad>', length=1024, pad_to_multiple_of=None)
...
if mydataset =='devign':
    if TEST_ONLY:
        ...
        test_index=set()
        with open(f'data/finetune/devign/test{sample_text}.txt') as f:
            for line in f:
                line=line.strip()
                test_index.add(int(line))
        mydata = pd.read_json(f'data/finetune/devign/Devign{sample_text}.jsonlines', lines=True)
        ...
        with open(f'data/finetune/devign/train{sample_text}.txt') as f:
            for line in f:
                line=line.strip()
                train_index.add(int(line))

        with open(f'data/finetune/devign/valid{sample_text}.txt') as f:
            for line in f:
                line=line.strip()
        ...
        mydata = read_jsonl(f'data/finetune/devign/Devign{sample_text}.jsonlines')
        # mydata = pd.read_json(f'data/finetune/devign/Devign{sample_text}.jsonlines', lines=True)
    ...
    m2 = pd.read_json('data/finetune/combined/' + args.data_folder + '/valid.jsonl', lines=True)
    ...
    if TEST_ONLY:
        ...
        test_index = set()
        with open(f'data/finetune/msr/test{sample_text}.txt') as f:
            for line in f:
                line = line.strip()
                test_index.add(int(line))
        mydata = pd.read_json(f'data/finetune/msr/MSR_data_cleaned{sample_text}.jsonlines', lines=True)
        m3 = mydata.iloc[list(test_index)]
        mydata = None
        del (mydata)
        ...
        with open(f'data/finetune/msr/train{sample_text}.txt') as f:
            for line in f:
                line = line.strip()
                train_index.add(int(line))

        with open(f'data/finetune/msr/valid{sample_text}.txt') as f:
            for line in f:
                line = line.strip()
                valid_index.add(int(line))
        ...
        if generate_jsons == True:
            ...
            m1.to_json(f'data/finetune/msr/train{sample_text}.jsonlines', orient='records', lines=True)
            m2.to_json(f'data/finetune/msr/valid{sample_text}.jsonlines', orient='records', lines=True)
            m3.to_json(f'data/finetune/msr/test{sample_text}.jsonlines', orient='records', lines=True)
        """


        # m1 = pd.read_json(f'data/finetune/msr/train{sample_text}.jsonlines', orient='records', lines=True)
        # m2 = pd.read_json(f'data/finetune/msr/valid{sample_text}.jsonlines', orient='records', lines=True)
        # m3 = pd.read_json(f'data/finetune/msr/test{sample_text}.jsonlines', orient='records', lines=True)
        m1 = read_jsonl(f'data/finetune/msr/train{sample_text}.jsonlines')
        m2 = read_jsonl(f'data/finetune/msr/valid{sample_text}.jsonlines')
        m3 = read_jsonl(f'data/finetune/msr/test{sample_text}.jsonlines')
...
if mydataset == 'msr':
    if TEST_ONLY:
        ...
        train_encodings = encode(m1.func_before)

        # m1.func_after = m1.func_after.apply(cleaner)
        # train_encodings_not_vuln = encode(m1.func_after)

        # try:
        train_encodings = [{'func':enc.ids,'target':t} for enc, t in zip(train_encodings_vuln, m1.vul)]
            # train_encodings_not_vuln = [{'func': enc.ids, 'target':1} for enc in train_encodings_not_vuln]
        # except:
        #     train_encodings = [{'func':enc.ids,'target':0} for enc in train_encodings_vuln]
            # train_encodings_not_vuln = [{'func': enc.ids, 'target':1} for enc in train_encodings_not_vuln]

        m2.func_before = m2.func_before.apply(cleaner)
        val_encodings = encode(m2.func_before)

        # m2.func_after = m2.func_after.apply(cleaner)
        # val_encodings_not_vuln = encode(m2.func_after)
        # try:
        val_encodings = [{'func': enc.ids, 'target': 0} for enc in zip(val_encodings_vuln, m2.vul)]
            # val_encodings_not_vuln = [{'func': enc.ids, 'target': 1} for enc in val_encodings_not_vuln]
        # except:
        #     val_encodings = [{'func': enc.ids, 'target': 0} for enc in val_encodings_vuln]
            # val_encodings_not_vuln = [{'func': enc.ids, 'target': 1} for enc in val_encodings_not_vuln]

        m3.func_before = m3.func_before.apply(cleaner)
        test_encodings = encode(m3.func_before)

        # m3.func_after = m3.func_after.apply(cleaner)
        # test_encodings_not_vuln = encode(m3.func_after)
        # try:
        test_encodings = [{'func': enc.ids, 'target': 0} for enc in zip(test_encodings_vuln, m3.vul)]
    ...
    if TEST_ONLY:
        m3.func = m3.func.apply(cleaner)
        test_encodings = encode(m3.func.tolist())
        try:
            test_encodings = [{'func':enc.ids,'target':lab} for enc,lab in zip(test_encodings,m3.target.tolist())]
        except:
            test_encodings = [{'func':enc.ids,'target':lab} for enc,lab in zip(test_encodings,(m3['combine']*1).tolist())]
        ...
        train_encodings = encode(m1.func.tolist())
        try:
        ...
        val_encodings = encode(m2.func.tolist())
        try:
...
print("Tensorizing...")
CODES = data.Field(batch_first=True, fix_length=1024,use_vocab=False, pad_token=pad_id)
...
def evaluate_dataset(dataset, batch_size=args.batch):
    ...
    i = 0
    for row in all_prob:
        all_prob[i] = torch.nn.functional.softmax(torch.tensor(row), dim=0)[1].item()
        i+=1

    results = pd.DataFrame(all_prob)
    results = results.rename(columns={0: "prob"})
    predicted_labels_df = pd.DataFrame(all_pred)
    predicted_labels_df = predicted_labels_df.rename(columns={0: "pred"})
    actual_labels_df = pd.DataFrame(all_labels)
    actual_labels_df= actual_labels_df.rename(columns={0: "target_"})

    results = pd.concat([results, predicted_labels_df, actual_labels_df, all_indices], axis="columns")
