...
def test(args, model, tokenizer):
    ...
    labels=np.concatenate(labels,0)
    preds=logits[:,0]>0.5
    preds=[1 if x else 0 for x in preds]
    ...
    if args.save_results:
        ...
        df['prob'] =  logits[:, 0]
        df['target'] = labels
        df.to_csv(os.path.join(args.result_dir, 'bugs', 'predictions_{}.csv'.format(args.dataset)))
...
BASE_DIR = '<ANONYMOUS>/<ANONYMOUS>/data/RQ.A2/'
SAVED_DIR = '<ANONYMOUS>/<ANONYMOUS>/saved_models/regvd/RQA2/'
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, required=True, help='Name of the dataset for experiment.')
    parser.add_argument('--test', type=str, required=True, help='Name of the dataset for experiment.')
