...
def get_sentences(_path,labelpath,deletepath,corpuspath,maptype=True):
    ...
    for filename in os.listdir(_path):
        ...
        f1.close()
        print("slice_list ", len(slicelists))
        ...
        for slicelist in slicelists:
            ...
            if not(lastprogram_id == program_id):
                ...
                save_file_name = filename[:-4] + '.pkl'
                if lastprogram_id not in os.listdir(corpuspath):    
                    os.mkdir(folder_path)
                if save_file_name not in os.listdir(folder_path):    
            ...
            focuspointer[0] = " ".join(sentences[0].split(" ")[2:-1])
            ...
            for sentence in sentences:
                ...
                if flag_focus == 1:
                    if "expr" in filename:
                        ...
                        def focusPointerIndex(list_tokens, fp):
                            fp_ = fp
                            if fp[0] == '[' and fp[-1] == ']':
                                fp_ = fp[1:-1]
                            fp_ = fp_.split(", ")
                            for y in fp_:
                                ...
                                if x[0] == '*':
                                    x = x[1:].strip()
                                if x in sentence:
                                    indi = sentence.index(x)
                                    return len(create_tokens(sentence[:indi]))
                            return -1

                        ind_ = focusPointerIndex(list_tokens, focuspointer[0])             
                        if ind_ != -1:
                            focus_index = focus_index + ind_
                            flag_focus = 2  
                            slicefile_focus.append(focus_index)
                        else:  
            ...
            if flag_focus == 0:
                continue
            # print(file_name, slicefile_corpus)
            slicefile_labels.append(labellists[int(file_name.split()[0])-1])
        ...
        if save_file_name not in os.listdir(folder_path):    
            ...
            pickle.dump([slicefile_corpus+data[0],slicefile_labels+data[1],slicefile_focus+data[2],slicefile_func+data[3],slicefile_filenames+data[4]], f1)
        f1.close()
        print(filename, delete_koyta, roiche_koyta)

if __name__ == '__main__':
    
    SLICEPATH = '<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/slice_files/'
    LABELPATH = '<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/slice_labels/'
    DELETEPATH = '<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/delete_list'
    CORPUSPATH = '<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/corpus/'
    MAPTYPE = True
