...
'''
def dealrawdata(raw_traindataSet_path, raw_testdataSet_path, traindataSet_path, testdataSet_path, batch_size, maxlen, vector_dim, split_ratio):
    print("Loading data...")
    
    modes = ["api", "arraysuse", "pointersuse", "integeroverflow"]
    split_ratio //= 10 #Only works if split ratio is a multiple of 10
    for mode in modes:
        train = [[], [], [], [], []]
        test = [[], [], [], [], []]
        cnt = -1
        for filename in os.listdir(raw_traindataSet_path):
            if not filename.endswith(".pkl"):
                continue
            if mode in filename:
                data = load_data_binary(raw_traindataSet_path + filename, batch_size, maxlen=maxlen, vector_dim=vector_dim)
                for sample in zip(*data):
                    cnt = (cnt + 1) % 10
                    if cnt < split_ratio:
                        for i in range(5):
                            test[i].append(sample[i])
                    else:
                        for i in range(5):
                            train[i].append(sample[i])
                

        for filename in os.listdir(raw_testdataSetPath):
            if not filename.endswith(".pkl"):
                continue
            if mode in filename:
                data = load_data_binary(raw_testdataSet_path + filename, batch_size, maxlen=maxlen, vector_dim=vector_dim)
                for sample in zip(*data):
                    cnt = (cnt + 1) % 10
                    if cnt < split_ratio:
                        for i in range(5):
        ...
        print(mode, len(train[1]), len(test[1]))
        f = open(os.path.join(traindataSet_path, mode + "_.pkl"), "wb")
        pickle.dump(train, f)
        f.close()

        f = open(os.path.join(testdataSet_path, mode + "_.pkl"), "wb")
        pickle.dump(test, f)
        f.close()
...
if __name__ == "__main__":
    ...
    maxLen = 500
    split_ratio = 10
    raw_traindataSetPath = "<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/dl_input/train/"
    raw_testdataSetPath = "<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/dl_input/test/"
    traindataSetPath = "<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/final_data/train/"
    testdataSetPath = "<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/final_data/test/"
    dealrawdata(raw_traindataSetPath, raw_testdataSetPath, traindataSetPath, testdataSetPath, batchSize, maxLen, vectorDim, split_ratio)
    print("dealrawdata success")
