...
def main():
    ...
    lang = '<ANONYMOUS>/<ANONYMOUS>/sysevr/data/SARD/'
    
    path = os.path.join(lang,'slice_files', 'api_slices.txt')
    list_all_apilabel = make_label(path, _dict)
    dec_path = os.path.join(lang, 'slice_labels', 'api_slices.pkl')
    f = open(dec_path, 'wb')
    pickle.dump(list_all_apilabel, f, True)
    f.close()
    
    path = os.path.join(lang,'slice_files', 'arraysuse_slices.txt')
    list_all_arraylabel = make_label(path, _dict)
    dec_path = os.path.join(lang, 'slice_labels', 'arraysuse_slices.pkl')
    f = open(dec_path, 'wb')
    pickle.dump(list_all_arraylabel, f, True)
    f.close()
    
    path = os.path.join(lang, 'slice_files', 'pointersuse_slices.txt')
    list_all_pointerlabel = make_label(path, _dict)
    dec_path = os.path.join(lang, 'slice_labels', 'pointersuse_slices.pkl')
    f = open(dec_path, 'wb')
    pickle.dump(list_all_pointerlabel, f, True)
    f.close()
 
    path = os.path.join(lang, 'slice_files', 'integeroverflow_slices.txt')
    list_all_exprlabel = make_label(path, _dict)
    dec_path = os.path.join(lang, 'slice_labels', 'integeroverflow_slices.pkl')
    f = open(dec_path, 'wb')
