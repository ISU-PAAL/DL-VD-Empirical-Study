...
def getAllFuncfileByTestID(db, testID):
    print "Starting getAllFuncfileByTestID()"
    testID = '*/'+ testID + '/*'
    query_str = "queryNodeIndex('type:File AND filepath:%s').id" % testID
    #print testID, db, query_str
    print "Query Started"
    results = db.runGremlinQuery(query_str)
    print "Query ended"
    #print(results)
    print "Ending getAllFuncfileByTestID()"
    return results
...
def getFuncNodeInTestID(db, testID):
    print "Starting getFuncNodeInTestID(): "
    ...
    print "Ending getFuncNodeInTestID(): "
...
def getCallGraph(db, testID):
    print("Starting getcallGraph: ")
    list_all_func_node = getFuncNodeInTestID(db, testID)
    print "Inside getCallGraph():-call_1"
...
if __name__ == '__main__':
    ...
    for testID in list_testID:
        ...
        #     continue
        num = int(testID)
        if num < 900 or num >= 1000:
            continue
        print testID
        f1 = open("./outputs/who_is_running_9.txt", "a")
        f1.write(str(testID) + "\n")
        f1.close()
