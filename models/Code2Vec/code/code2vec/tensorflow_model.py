        ...
        try:
            while True:
                ...
                if batch_num % num_batches_to_save_and_eval == 0:
                    ...
                    model_save_path = self.config.MODEL_SAVE_PATH + '_iter' + str(epoch_num) + "_performance_" + evaluation_results.get_cool_slug()
                    self.save(model_save_path)
                    self.log('Saved after %d epochs in: %s' % (epoch_num, model_save_path))
                    #------END
        except tf.errors.OutOfRangeError:
            print("The reader iterator is exhausted and have no more batches to produce.")
