...
class DevignModel(nn.Module):
    ...
    def forward(self, batch, feat=None, cuda=False, save_after_ggnn=False, eweight=None):
        graph, features, edge_types = batch.get_network_inputs(cuda=cuda, device="cuda:0")
        if feat != None:
            features = feat
        outputs = self.ggnn(graph, features, edge_types, eweight=eweight)
...
class GGNNSum(nn.Module):
    ...
    def forward(self, batch, cuda=False, save_after_ggnn=False):
        graph, features, edge_types = batch.get_network_inputs(cuda=cuda, device="cuda:0")
