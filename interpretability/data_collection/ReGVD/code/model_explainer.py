...
class GNNReGVD(nn.Module):
    ...
    def forward(self, adj, x_feature, labels=None, edge_mask=None):
        # construct graph
        ...
        if edge_mask is not None:
            # print("Yes, there")
            rows, cols = adj_copy.nonzero()
            for i, j, mu in zip(rows, cols, edge_mask):
                # print(i, j, mu)
                adj[0][i][j] = adj[0][i][j] * mu
        # print("adj: ", adj.shape, type(adj), adj_mask)
        # run over GNNs
