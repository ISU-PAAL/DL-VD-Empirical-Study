...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):       
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), self.examples[i].idx, self.examples[i].input_tokens
...
def test(args, model, tokenizer):
    ...
    w_embeddings = model.encoder.roberta.embeddings.word_embeddings.weight.data.cpu().detach().clone().numpy()
    ...
    for batch in eval_dataloader:
        ...
        if args.format == "uni":
            adj, x_feature, doc_word_id_map = build_graph(inputs.cpu().detach().numpy(), w_embeddings, window_size=args.window_size, return_id_map=True)
        else:
            adj, x_feature = build_graph_text(inputs.cpu().detach().numpy(), w_embeddings, window_size=args.window_size)
        
        x_feature = np.array(x_feature)
        feat_mask, edge_mask = explainer.explain_graph(adj, x_feature)
        
        ids_ = inputs[0].cpu().detach().numpy()
        tokens_ = tokenizer.convert_ids_to_tokens(ids_)
        id_to_tokens = {}
        for i in range(len(tokens_)):
            id_to_tokens[ids_[i]] = tokens_[i]
        
        node_to_tokens = {v : id_to_tokens[k] for k, v in doc_word_id_map.items()}
        rows, cols = adj[0].nonzero()

        src_nodes = [node_to_tokens[x] for x in rows]
        des_nodes = [node_to_tokens[x] for x in cols]
        assert len(src_nodes) == len(edge_mask)
        assert len(des_nodes) == len(edge_mask)
        results.append({
            'feat_mask': feat_mask,
            'edge_mask': edge_mask,
            'id': batch[2],
    ...
    print(results)
    pickle.dump(results, open("explanation_devign_results.pkl", "wb"))
    return results
...
def main(args):
    ...
    print("load roberta pretrained model: ")
...
class Args:
    def __init__(self):
        self.train_data_file = "<ANONYMOUS>/<ANONYMOUS>/data/devign/train.jsonl"
        self.eval_data_file = "<ANONYMOUS>/<ANONYMOUS>/data/devign/valid.jsonl"
        self.test_data_file = "<ANONYMOUS>/<ANONYMOUS>/data/devign/test.jsonl"
        self.output_dir = "<ANONYMOUS>/<ANONYMOUS>/saved_models/regvd/devign/312456/"
        ## Other parameters
