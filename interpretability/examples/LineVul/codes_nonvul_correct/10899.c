// commit message qemu@ffad4116b9 (target=0, prob=0.46456242, correct=True): net: Fix -net socket parameter checks
/*0   */ int drive_init(struct drive_opt *arg, int snapshot, void *opaque)                  // (6) 0.03907
/*2   */ {                                                                                  // (25) 0.001956
...
/*16  */     enum { MEDIA_DISK, MEDIA_CDROM } media;                                        // (7) 0.03906
...
/*34  */     int bdrv_flags, onerror;                                                       // (9) 0.0254
...
/*48  */                                            NULL };                                 // (3) 0.08593
/*52  */     if (check_params(buf, sizeof(buf), params, str) < 0) {                         // (5) 0.04492
...
/*606 */     strncpy(drives_table[nb_drives].serial, serial, sizeof(serial));               // 0.0
/*608 */     nb_drives++;                                                                   // 0.0
/*612 */     switch(type) {                                                                 // 0.0
/*614 */     case IF_IDE:                                                                   // 0.0
...
/*666 */     if (!file[0])                                                                  // 0.0
/*668 */         return -2;                                                                 // 0.0
/*670 */     bdrv_flags = 0;                                                                // 0.0
/*672 */     if (snapshot) {                                                                // 0.0
/*674 */         bdrv_flags |= BDRV_O_SNAPSHOT;                                             // 0.0
/*676 */         cache = 2; /* always use write-back with snapshot */                       // 0.0
/*678 */     }                                                                              // 0.0
/*680 */     if (cache == 0) /* no caching */                                               // 0.0
/*682 */         bdrv_flags |= BDRV_O_NOCACHE;                                              // 0.0
/*684 */     else if (cache == 2) /* write-back */                                          // 0.0
/*686 */         bdrv_flags |= BDRV_O_CACHE_WB;                                             // 0.0
/*688 */     else if (cache == 3) /* not specified */                                       // 0.0
/*690 */         bdrv_flags |= BDRV_O_CACHE_DEF;                                            // 0.0
/*692 */     if (bdrv_open2(bdrv, file, bdrv_flags, drv) < 0) {                             // 0.0
/*694 */         fprintf(stderr, "qemu: could not open disk image %s\n",                    // 0.0
/*696 */                         file);                                                     // 0.0
/*698 */         return -1;                                                                 // 0.0
/*700 */     }                                                                              // 0.0
/*702 */     if (bdrv_key_required(bdrv))                                                   // 0.0
/*704 */         autostart = 0;                                                             // 0.0
/*706 */     return drives_table_idx;                                                       // 0.0
/*708 */ }                                                                                  // 0.0
